...
HOTELS_DB = {
    ...
    "Phú Quốc": [
        {"name": "Vinpearl Resort", "stars": 5, "price_per_night": 3500000, "area": "Bãi Dài", "rating": 4.7},
        {"name": "Sol by Meliá", "stars": 4, "price_per_night": 1500000, "area": "Bãi Trường", "rating": 4.4},
        {"name": "Lahana Resort", "stars": 3, "price_per_night": 800000, "area": "Dương Đông", "rating": 4.3},
        {"name": "9Station Hostel", "stars": 2, "price_per_night": 200000, "area": "Dương Đông", "rating": 4.5},
    ],

    "Hồ Chí Minh": [
        {"name": "Rex Hotel", "stars": 5, "price_per_night": 2800000, "area": "Quận 1", "rating": 4.4},
        {"name": "Liberty Central", "stars": 4, "price_per_night": 1400000, "area": "Quận 1", "rating": 4.2},
        {"name": "Cochin Zen Hotel", "stars": 3, "price_per_night": 550000, "area": "Quận 3", "rating": 4.0},
...
def search_flights(origin: str, destination: str) -> str:
    """Tìm chuyến bay tốt nhất (rẻ nhất)"""
    ...
    return (
        f"✈️ Tốt nhất:\n"
        f"{best['airline']} | {best['departure']} → {best['arrival']} | {format_price(best['price'])}\n\n"
        f"📋 Tất cả:\n" +
        "\n".join([
            f"{f['airline']} | {f['departure']} → {f['arrival']} | {format_price(f['price'])}"
            for f in flights
        ])
    )
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    if not filtered:
        return "Không có khách sạn phù hợp"

    # ✅ chọn tốt nhất theo rating/price
    best = sorted(filtered, key=lambda x: (-x["rating"], x["price_per_night"]))[0]

    return (
        f"🏨 Gợi ý tốt nhất:\n"
        f"{best['name']} ({best['stars']}⭐) | {format_price(best['price_per_night'])} | ⭐{best['rating']}\n\n"
        f"📋 Danh sách:\n" +
        "\n".join([
            f"{h['name']} ({h['stars']}⭐) | {format_price(h['price_per_night'])}"
            for h in filtered
        ])
    )

@tool
def calculate_budget(flight_price: int, hotel_price: int, nights: int, total_budget: int) -> str:
    ...
    return (
        f"💰 Chi phí:\n"
        f"- Vé máy bay: {format_price(flight_price)}\n"
        f"- Khách sạn ({nights} đêm): {format_price(hotel_price * nights)}\n"
        f"👉 Tổng: {format_price(total_cost)}\n"
        f"👉 Còn lại: {format_price(remaining)}"
    )